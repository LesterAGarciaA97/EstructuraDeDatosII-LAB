...
namespace Lab03_EDII.Lzw
{
    public class DescompresionLZW
    {
        ...
        public string DescomprimirLZW(string filename, string path)
        {
            ...
            var agrupados = Agrupar(temp, tamanio);
            var descompress = Descompress(agrupados);
            WriteDescompress(descompress, path, filename);
            return Path.GetFileNameWithoutExtension(filename) + "FileDescompress.txt";
        }
        ...
        public List<string> ReadFile(string textname, string filepath)
        {
            ...
            tamanio = Convert.ToInt32(parts[parts.Length - 1]);
        ...
        {
            var path = Path.Combine(filepath, System.IO.Path.GetFileNameWithoutExtension(textname) + "FileDescompress.txt");
