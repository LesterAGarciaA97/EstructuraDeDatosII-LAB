...
namespace LaboratorioReposicionEDII.Class.Huffman
{
    public class Compresion
    {
        ...
        private static nodoMinimo[] CompresionDelArbol(Nodo nodoRaizArbol)
        {
            ...
            for (int i = 0; i < NodoResultante.Length; i++)
            {
                ...
                {
                    int left = (ListadoDeNodos[i].left == null) ? -1 : ListadoDeNodos[i].left.id;
                    int right = (ListadoDeNodos[i].right == null) ? -1 : ListadoDeNodos[i].right.id;
                    NodoResultante[i] = new nodoMinimo(ListadoDeNodos[i].letra, left, right);
                }
